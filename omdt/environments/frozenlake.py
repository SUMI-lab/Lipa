...
def generate_mdp(
    ...
    for state, action_P in env.unwrapped.P.items():
    ...
    observations[:, 0] = np.mod(np.arange(n_states), env.unwrapped.nrow)
    observations[:, 1] = np.floor_divide(np.arange(n_states), env.unwrapped.nrow)
    feature_names = ["X", "Y"]
